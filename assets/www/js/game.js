...
var Game = new (function() {
  var self = this,
      ...
      currentPuzzle = null,
      checkTOH = 0,
      ojoos = ['অসাধারণ','অতুলনীয়','অতুলনীয় ','অসাধারণ','অতুলনীয়','অসাধারণ','অতুলনীয়','অতুলনীয়','অসাধারণ','অতুলনীয়','অসাধারণ','অতুলনীয়','অতুলনীয়','অসাধারণ','অতুলনীয়','অতুলনীয়','অসাধারণ','অতুলনীয়','অতুলনীয়','অসাধারণ','অতুলনীয়','অতুলনীয়','অসাধারণ'],
      remainingOjoos = [],
      endGameTOH1,
      endGameTOH2,
      endGameTOH3,
  ...
  function showSizes() {
    onHomeScreen = false;
    showGame();
    $('#boardsize').html('<span >বোর্ড সিলেক্ট <br>করুন..</span>');
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone)
        grid.hint.show('এটি আনডু বাটন');
      else
        grid.hint.show('আনডু করার কিছু নেই');
      return;
    }
    ...
    tile.mark();
    var s = 'টাইলটি পরিবর্তিত হয়ছে ';
    if (value == 1) s += 'লাল এ।';
    if (value == 2) s += 'নীল  এ';
    if (value == 0) s += 'প্রাথমিক অবস্থায়।'
    grid.hint.show(s);
    undone = true;
    clearTimeout(checkTOH);
    checkTOH = setTimeout(function(){checkForLevelComplete();}, 700);
  ...
  function updateTweetUrl(size) {
    var msg = '#ব্লকপাজল I just completed a ' + size + ' x ' + size + ' puzzle and my score is ' + getScore() + '. ',
